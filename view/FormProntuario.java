...
public class FormProntuario extends FormEntidade
{
    ...
    private JComboBox<String> cbNome_medico;
    ...
    public FormProntuario (JFrame form, Database database, String n)
    {
        formPai = form;
    ...
    {       
        ...
        titlePrincipal = new JLabel("Prontuário");         
        ...
        // gbc.gridy = 0;
        // gbc.gridwidth = 1;                  
        // subPainel_2.add(new JLabel("ID:",SwingConstants.RIGHT),gbc);             
        // txtId = new JTextField();
        // txtId.setEnabled(false);
        // gbc.gridx = 1;
        // gbc.gridy = 0;
        // gbc.gridwidth = 1;          
        // subPainel_2.add(txtId,gbc);                     
        ...
        cbNome_medico = new JComboBox<>();
        // cbNome_medico.setEnabled(false);
        cbNome_medico.addActionListener(this);

        
        ArrayList<String> arrayM = carregarMedicos();
        cbNome_medico.setModel(new DefaultComboBoxModel<String>(arrayM.toArray(new String[0])));
               
        gbc.gridx = 1;
        gbc.gridy = 0;
        gbc.gridwidth = 6;                  
        subPainel_2.add(cbNome_medico,gbc);             
    ...
    public ArrayList<String> carregarMedicos(){
        ArrayList<String> array = new ArrayList<>();
        array_med_ids = new ArrayList<Integer>();
        for(Medico p: this.medicoList){
            array.add(p.getNome());
            array_med_ids.add(p.getId());
        }        
        return array;
    }
    
    public Paciente getPacienteSelecionado(Integer id){
        Paciente pacienteSelec;
        pacienteSelec = pacienteController.readFromId(array_pac_ids.get(id));
 
        txtCPF.setText(pacienteSelec.getCPF());
        if(pacienteSelec.getSexo().equals("Feminino")){
        ...
        String dataArr[] = pacienteSelec.getDataNascimento().split("/");
    ...
    public Medico getMedicoSelecionado(Integer id){
        Medico medicoSelec;       
        medicoSelec = medicoController.readFromId(array_med_ids.get(id));
        
        txtCRM.setText(medicoSelec.getCRM());
        selectItemByString(medicoSelec.getEstado());
    ...
    public void actionPerformed(ActionEvent e) 
    {
        ...
        {        
            getPacienteSelecionado(cbNome_paciente.getSelectedIndex());
        }
        
        else if(e.getSource().equals(cbNome_medico))
        {        
            getMedicoSelecionado(cbNome_medico.getSelectedIndex());
        }
    }

    public Prontuario getEntidadeCorrente()
    {
        Paciente paciente = getPacienteSelecionado(cbNome_paciente.getSelectedIndex());
        Medico medico = getMedicoSelecionado(cbNome_medico.getSelectedIndex());
        
        Prontuario prontuario = new Prontuario(medico, paciente);
        ...
        if(rbSim.isSelected()){
            prontuario.setTesteCovid("Positivo");
        } else {
            prontuario.setTesteCovid("Negativo");
        }
        
        prontuario.setRelato(txtRelato.getText()+"");
        
        String dataConsulta = cbDia_consulta.getSelectedItem() + "/" + cbMes_consulta.getSelectedItem() + "/" + cbAno_consulta.getSelectedItem();
    ...
    public void salvarEntidade()
    {
        ...
        prontuario = getEntidadeCorrente();        
        Prontuario prontuarioCriado = controller.create(prontuario);
    }
    ...
    public void exibirEntidade(int index)
    {
        ...
        else
        {
            ...
            selectDataConsultaByString(cbNome_medico, medico.getNome());
    ...
    {
        cbNome_paciente.setEnabled(true);
        cbNome_medico.setEnabled(true);
        cbDia_consulta.setEnabled(true);
        cbMes_consulta.setEnabled(true);
        cbAno_consulta.setEnabled(true);
        rbSim.setEnabled(true);
        rbNao.setEnabled(true);
        txtRelato.setEnabled(true);
    }*/
